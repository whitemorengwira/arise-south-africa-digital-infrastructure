...
# Project: Arise South Africa (Political · Integrated Multi-Channel Campaign & Platform)
# ------------------------------------------------------------------------------
...
provider "aws" {
  ...
  default_tags {
    tags = {
      Project     = "Arise South Africa"
      Network     = "Political · Integrated Multi-Channel Campaign & Platform"
...
resource "aws_kms_key" "media_key" {
  description             = "KMS Key for Arise South Africa media asset encryption"
  deletion_window_in_days = 30
  enable_key_rotation     = true

  tags = {
    Name = "arise_south_africa_digital_infrastructure-kms-key"
  }
}

# 2. S3 Media Vault Bucket
resource "aws_s3_bucket" "media_vault" {
  bucket        = "nwhite-arise-south-africa-digital-infrastructure-${var.environment}"
  force_destroy = false
}
...
resource "aws_iam_role" "post_production_role" {
  name = "arise_south_africa_digital_infrastructure-post-production-role"
...
resource "aws_iam_policy" "post_production_policy" {
  name        = "arise_south_africa_digital_infrastructure-post-production-policy"
  description = "Scoped S3 and KMS permissions for Arise South Africa post-production workflow"
...
resource "aws_cloudwatch_log_group" "workflow_logs" {
  name              = "/aws/media-pipeline/arise_south_africa_digital_infrastructure"
